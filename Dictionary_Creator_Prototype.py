...
class WordCounterApp:
    def __init__(self, root):
        ...
        self.original_words = []  # To store the original extracted words
        # Buttons
        ...
        self.result_tree.bind("<Button-3>", self.show_context_menu)  # Right-click menu
        ...
        self.context_menu.add_command(label="Delete Word", command=self.delete_selected_word)
    ...
    async def process_files(self):
        ...
        self.original_words = [word for word, _ in sorted_words] #store the original extracted words before any user edits


        # Insert results into the treeview, applying word limit
        total_words = min(len(sorted_words), word_limit if word_limit > 0 else len(sorted_words))
        self.progress_bar["value"] = 0
    ...
    def show_context_menu(self, event):
            """Displays the context menu for deleting words."""
            try:
                item = self.result_tree.identify_row(event.y)
                if item:
                    self.result_tree.selection_set(item)  # Select the clicked item
                    self.context_menu.post(event.x_root, event.y_root) #displays the right click menu
            except:
                pass


    def delete_selected_word(self):
        """Deletes the selected word from the Treeview."""
        selected_items = self.result_tree.selection()
